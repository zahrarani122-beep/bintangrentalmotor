    ...
    protected function afterCreate(): void
{
    ...
    if (!$pelanggan) {
        Notification::make()->title('Notifikasi gagal')->body('Data pelanggan tidak ditemukan.')->danger()->send();
        return;
    }
    ...
    $namaMotor = $penyewaan->penyewaanMotor->pluck('motor.nama_motor')->filter()->implode(', ') ?: '-';
    ...
    if ($statusDenda === 'Ada Denda' && count($detailDenda) > 0) {
        foreach (array_values($detailDenda) as $index => $item) {
            $pesan .= "Denda " . ($index + 1) . ":\n";
            $pesan .= "- Jenis Denda : " . ($item['jenis_denda'] ?? '-') . "\n";
            $pesan .= "- Nama Denda : " . ($item['nama_denda'] ?? '-') . "\n";
            $pesan .= "- Nominal : Rp" . number_format((float)($item['nominal'] ?? 0), 0, ',', '.') . "\n";
        }
    } else {
    ...
    if ($nomorTujuan) {
        ...
        if (($proses['status'] ?? false) == true) {
            Notification::make()->title('Notifikasi WhatsApp terkirim')->body('Pesan pengembalian berhasil dikirim.')->success()->send();
        } else {
            Notification::make()->title('Notifikasi WhatsApp gagal')->body($proses['reason'] ?? 'Gagal mengirim pesan.')->danger()->send();
        }
    } else {
        Notification::make()->title('WhatsApp tidak dikirim')->body('Nomor HP pelanggan tidak tersedia.')->warning()->send();
    }
}
}